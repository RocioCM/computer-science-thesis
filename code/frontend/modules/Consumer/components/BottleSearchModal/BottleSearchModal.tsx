...
const BottleSearchModal = ({
  ...
  const handleSearch = async () => {
    const { ok, data } = await ConsumerServices.searchBottle(form.search);
    if (ok) {
        ...
        <>
          ...
          <div className="h-full w-full overflow-auto hide-scroll relative pt-2">
            ...
            <div className="grid grid-cols-2 gap-3xl">
              ...
              <Button
                handleClick={() =>
                  handleRecycle(form.product.trackingCode || form.search)
                }
                width="full"
                className="mt-6"
              >
                Reciclar
                <FaIcon type="fa-solid fa-arrow-right" />
              </Button>
            </div>
          </div>
        </>
